"""
This Python code defines the ItemLoader class, which processes product data from the Amazon Reviews 2023 dataset. It efficiently loads, filters, and processes data using parallel execution.
Dependencies:
- Uses datasets.load_dataset to fetch the dataset.
- ProcessPoolExecutor speeds up processing via multiprocessing.
- tqdm provides a progress bar for better visualization.
- Imports the Item class for data cleaning and tokenization.

Class: ItemLoader
- from_datapoint: Converts raw dataset entries into Item objects if their price falls within the allowed range (0.5 to 999.49).
- from_chunk: Processes chunks of data points and filters out valid Item objects.
- chunk_generator: Iterates over the dataset in chunks (CHUNK_SIZE = 1000).
- load_in_parallel: Uses multiprocessing to process chunks concurrently, improving efficiency.
- load: Loads the dataset, processes it using multiple workers (default: 8), and logs execution time.
"""
...
class ItemLoader:
    ...
    def from_datapoint(self, datapoint):
        """
        Create an Item from this datapoint
